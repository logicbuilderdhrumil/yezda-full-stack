    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className={cn(
            'flex items-center gap-2 rounded-md p-2 text-[var(--color-muted-foreground)] hover:bg-[var(--color-muted)] hover:text-[var(--color-foreground)] transition-colors duration-200',
      ...
      <DropdownMenuContent align="end" className="w-44">
        ...
        {themeModeOptions.map(({ mode, labelKey, icon: Icon }) => (
          ...
          >
            ...
            {currentMode === mode && <Check className="h-4 w-4 text-[var(--color-cta)]" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
