/**
 * Client sidebar navigation component with role-based filtering.
 * Used for the client portal routes (/) for screening clients and org staff.
 */
...
import { clientNavConfig, iconMap } from '@/configs';
...
/**
 * ClientSidebar component with navigation items filtered by user role.
 */
export function ClientSidebar({ className }: ClientSidebarProps): ReactNode {
  const { t } = useTranslation();
  const { user } = useAuth();
  ...
  const filteredSections = useMemo(
    () => filterNavConfig(clientNavConfig.sections, user?.roles),
    [user?.roles]
  );
