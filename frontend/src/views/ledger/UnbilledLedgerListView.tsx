/**
 * Unbilled ledger list view with filters, totals, and pagination.
 */
...
import type { LedgerEntry, LedgerEntryStatus, LedgerListParams, LedgerSummary } from '@/@types/ledger';

const DEFAULT_PAGE_SIZE = 10;

/**
 * Returns badge variant for ledger entry status.
 */
function getStatusVariant(status: LedgerEntryStatus): 'default' | 'secondary' | 'destructive' {
  switch (status) {
    case 'pending':
      return 'secondary';
    case 'unbilled':
...
/**
 * UnbilledLedgerListView displays unbilled ledger entries with summary and filters.
 */
export function UnbilledLedgerListView(): ReactNode {
  ...
  const sortBy = (searchParams.get('sortBy') as LedgerListParams['sortBy']) || 'createdAt';
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const response = await LedgerService.listUnbilled(params);
      setEntries(response.data);
      setTotalItems(response.meta.totalItems);
      setTotalPages(response.meta.totalPages);
      setSummary(response.summary);
    } catch (error) {
      toastError(t('ledger.unbilled.fetchError'));
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `unbilled-ledger-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  return (
    <PageContainer
      title={t('ledger.unbilled.title')}
      description={t('ledger.unbilled.description')}
    >
      {/* Summary Cards */}
      {summary && (
