/**
 * Certified candidates list view.
 */
...
/**
 * CertifiedCandidatesListView displays a paginated table of certified candidates.
 */
export function CertifiedCandidatesListView(): ReactNode {
  ...
  const sortBy = (searchParams.get('sortBy') as CandidateListParams['sortBy']) || 'certifiedAt';
  const sortOrder = (searchParams.get('sortOrder') as CandidateListParams['sortOrder']) || 'desc';

  const [searchInput, setSearchInput] = useState(search);

  // Fetch certified candidates
  const fetchCandidates = useCallback(async () => {
    ...
    try {
      const params: CandidateListParams = {
        ...
        status: 'certified', // Fixed filter for certified candidates
        sortBy,
        sortOrder,
      };
      ...
      toastError(t('candidates.certified.fetchError'));
      console.error('Failed to fetch certified candidates:', error);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <PageContainer
      title={t('candidates.certified.title')}
      description={t('candidates.certified.description')}
    >
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead
                  className="cursor-pointer select-none"
                  onClick={() => handleSort('certifiedAt')}
                >
                  {t('candidates.columns.certifiedAt')}
                  {renderSortIcon('certifiedAt')}
                </TableHead>
              ...
              {candidates.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-8 text-gray-500">
                    {t('candidates.certified.noResults')}
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  >
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="default">{t('candidates.status.certified')}</Badge>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-gray-500">
                      {candidate.certifiedAt ? formatDate(candidate.certifiedAt) : '-'}
                    </TableCell>
                    <TableCell>
                      <Button
                        variant="ghost"
