/**
 * Pipeline edit view - similar to create but pre-populated.
 * Only available for draft pipelines.
 */
...
/**
 * PipelineEditView renders the form to edit an existing draft pipeline.
 */
export function PipelineEditView(): ReactNode {
    ...
    const fetchPipeline = async () => {
      ...
      try {
        ...
        if (data.status !== 'draft') {
          toastError(t('pipelines.edit.notDraft', 'Only draft pipelines can be edited'));
        ...
        toastError(t('pipelines.edit.fetchError', 'Failed to load pipeline'));
        navigate('/admin/pipelines');
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const handleSave = async (activate: boolean = false) => {
    ...
    try {
      ...
      const updated = await PipelineService.update(id, payload);

      if (activate) {
        await PipelineService.activate(updated.id);
        toastSuccess(t('pipelines.edit.successActivated', 'Pipeline saved and activated'));
      } else {
        toastSuccess(t('pipelines.edit.success', 'Pipeline saved successfully'));
      }

      navigate(`/admin/pipelines/${id}`);
    } catch (error) {
      handleApiError(error);
      toastError(t('pipelines.edit.error', 'Failed to save pipeline'));
    } finally {
  ...
  if (isLoading) {
    return (
      <PageContainer title={t('pipelines.edit.title', 'Edit Pipeline')}>
        <SkeletonCard />
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer
      title={t('pipelines.edit.title', 'Edit Pipeline')}
      description={t('pipelines.edit.description', 'Modify the pipeline configuration')}
    >
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('pipelines.edit.infoTitle', 'Pipeline Information')}</CardTitle>
            <CardDescription>
              {t('pipelines.edit.infoDescription', 'Basic details about the pipeline')}
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>{t('pipelines.edit.stagesTitle', 'Pipeline Stages')}</CardTitle>
                <CardDescription>
                  {t('pipelines.edit.stagesDescription', 'Define the stages of the screening process')}
                </CardDescription>
              </div>
          ...
          <CardContent className="space-y-4">
            ...
            {stages.length === 0 ? (
              <p className="text-gray-500 text-center py-4">
                {t('pipelines.edit.noStages', 'No stages added yet. Click "Add Stage" to begin.')}
              </p>
            ) : (
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button variant="secondary" onClick={() => handleSave(false)} disabled={isSubmitting}>
            {isSubmitting ? t('common.saving', 'Saving...') : t('pipelines.edit.save', 'Save Changes')}
          </Button>
          <Button onClick={() => handleSave(true)} disabled={isSubmitting}>
            {isSubmitting ? t('common.saving', 'Saving...') : t('pipelines.edit.saveActivate', 'Save & Activate')}
          </Button>
        </div>
      </div>
    </PageContainer>
