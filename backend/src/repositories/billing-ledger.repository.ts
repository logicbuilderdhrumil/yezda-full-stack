/**
 * Billing Ledger Repository
 * Task 1.1: Ledger entry schema and storage queries
 */
...
export class BillingLedgerRepository {
  ...
  /**
   * Find billed ledger entries with filters
  ...
  /**
   * Find unbilled ledger entries with filters
  ...
  /**
   * Find entries by status with filters
   */
  private async findEntriesByStatus(
    ...
    if (filters.entryTypes && filters.entryTypes.length > 0) {
      ...
      values.push(filters.entryTypes);
    }
    ...
    const sortColumn = filters.sortBy === 'totalAmount'
      ? 'total_amount'
      : filters.sortBy === 'billedAt'
        ? 'billed_at'
        : 'created_at';
    ...
    return {
      entries: entriesResult.rows.map(rowToLedgerEntry),
      totalCount,
    };
  }

  /**
   * Calculate totals for ledger entries
   * Task 1.2, 1.3: Filters and totals
   */
  ...
  /**
   * Check if an entry is finalized (immutable)
   * Task 1.6: Immutability safeguards
   */
  async isEntryFinalized(
    tenantId: string,
    organizationId: string,
    id: string
  ): Promise<boolean> {
    const result = await query<{ finalized_at: Date | null }>(
      `SELECT finalized_at FROM ledger_entries
       WHERE tenant_id = $1 AND organization_id = $2 AND id = $3`,
      [tenantId, organizationId, id]
    );
  ...
  /**
   * Finalize a ledger entry (mark as immutable)
   * Task 1.6: Immutability safeguards
   */
  async finalizeEntry(
    tenantId: string,
    ...
    try {
      ...
      if (checkResult.rows[0]?.finalized_at) {
        ...
        return undefined; // Already finalized
      }
  ...
  /**
   * Attempt to update a finalized entry (should fail with immutability check)
   * Task 1.6: Immutability safeguards
   */
  async updateEntry(
    tenantId: string,
    ...
    try {
      ...
      const checkResult = await client.query<{ finalized_at: Date | null; status: string }>(
        `SELECT finalized_at, status FROM ledger_entries
         WHERE tenant_id = $1 AND organization_id = $2 AND id = $3
         FOR UPDATE`,
        [tenantId, organizationId, id]
      );
      ...
      if (setClauses.length === 0) {
        await client.query('ROLLBACK');
        const entry = await this.findById(tenantId, organizationId, id);
      ...
      return {
        success: true,
        entry: result.rows[0] ? rowToLedgerEntry(result.rows[0]) : undefined,
      };
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
