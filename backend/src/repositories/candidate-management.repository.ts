 ...
 * Task 1.2, 1.3, 1.4: Persistence layer for candidate management operations
...
export class CandidateManagementRepository {
  /**
  ...
  /**
   * Create multiple candidates in a single transaction
   */
  async bulkCreate(
  ...
  /**
   * Find candidate by ID with tenant scoping
   */
  ...
  /**
   * Find candidate by email with tenant scoping
   */
  ...
  /**
   * Update candidate with tenant scoping
   */
  async update(
    id: string,
    tenantId: string,
  ...
  /**
   * List and search candidates with pagination and tenant scoping
   * Task 1.4: Supports certified and archived filters
   */
  ...
  async updateStatus(
    ...
    const baseUpdate = { status, updatedBy };

    // Add special handling for certified and archived statuses
    if (status === 'certified') {
    ...
    return this.update(id, tenantId, baseUpdate);
  }

  /**
   * Certify a candidate
   */
  async certify(id: string, tenantId: string, certifiedBy?: string): Promise<ManagedCandidate | undefined> {
    const now = new Date();
    const result = await query<CandidateRow>(
   ...
   * Archive a candidate
   */
  async archive(id: string, tenantId: string, archivedBy?: string): Promise<ManagedCandidate | undefined> {
    const now = new Date();
    const result = await query<CandidateRow>(
      `UPDATE candidates SET 
        status = 'archived', 
  ...
  /**
   * Delete candidate (hard delete - use with caution)
   */
  ...
  /**
   * Check if tenant exists (for public submission validation)
   */
...
export const candidateManagementRepository = new CandidateManagementRepository();
