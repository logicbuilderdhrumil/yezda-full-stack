/**
 * User Management Rate Limit Middleware
...
// Rate limit configuration for user management
const USER_MANAGEMENT_RATE_LIMITS = {
  list: { windowMs: 60000, max: 60 },      // 60 requests per minute for list/search
  ...
  return async (req: AuthenticatedRoleRequest, res: Response, next: NextFunction) => {
    ...
    if (!redisCircuitOpen) {
      try {
        ...
        redisErrorCount += 1;
        metricsService.recordRedisError('user_management_rate_limit');

        console.error('[UserManagement] Rate limit Redis error:', error);
    ...
    if (!result!.allowed) {
      ...
      auditService.logAnomaly({
        description: `User management ${operation} rate limit exceeded`,
        channel: 'api',
        ipAddress: ip,
        metadata: {
          operation,
      ...
      metricsService.incrementCounter('user_management_rate_limit_hit', { operation });
...
export const userManagementListRateLimiter = createUserManagementRateLimiter('list');
export const userManagementCreateRateLimiter = createUserManagementRateLimiter('create');
export const userManagementUpdateRateLimiter = createUserManagementRateLimiter('update');
export const userManagementDeleteRateLimiter = createUserManagementRateLimiter('delete');
