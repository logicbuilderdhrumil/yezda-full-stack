/**
 * PostgreSQL User Management Repository
 *
 * Implements IUserManagementRepository using the shared PostgreSQL query function.
 * Enforces tenant isolation at the database query level.
 */
import type { IUserManagementRepository } from '../../domain/ports/user-management-repository.port.js';
import type {
  ManagedUser,
  CreateUserDto,
  UpdateUserDto,
  ...
  UserStatus,
  UserRole,
} from '../../domain/entities/user-management.entity.js';
...
export class PostgresUserManagementRepository implements IUserManagementRepository {
  async search(params: UserSearchParams & { tenantId: string }): Promise<UserListResult> {
    const {
      tenantId,
      query: searchQuery,
      status,
  ...
  async create(
    input: CreateUserDto & { id: string; tenantId: string; createdBy?: string; passwordHash?: string },
  ): Promise<ManagedUser> {
    const now = new Date();
    const result = await query<ManagedUserRow>(
  ...
  async update(
    id: string,
    tenantId: string,
    input: UpdateUserDto & { updatedBy?: string },
